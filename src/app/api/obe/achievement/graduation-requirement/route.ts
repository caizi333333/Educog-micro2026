...
export async function GET(request: NextRequest) {
  try {
    ...
    const where: Prisma.GraduationRequirementAchievementWhereInput = {};
    ...
    const configurationUpdatedAt = configurationTimes.length > 0
      ...
      : null;
    const activeIndicatorIds = [...new Set(activeObjectives.map((objective) => objective.indicatorPointId))];
    const currentWhere: Prisma.GraduationRequirementAchievementWhereInput = {
      ...where,
      indicatorPointId: { in: activeIndicatorIds },
      ...(configurationUpdatedAt ? { calculatedAt: { gte: configurationUpdatedAt } } : {}),
    };

    const achievements = await prisma.graduationRequirementAchievement.findMany({
      where: currentWhere,
    ...
    const targetCount = userId
      ...
      : classId
        ...
        : null;
    const staleRecords = configurationUpdatedAt && activeIndicatorIds.length > 0
      ? await prisma.graduationRequirementAchievement.count({
        where: {
          ...where,
          indicatorPointId: { in: activeIndicatorIds },
    ...
    const expectedRecords = targetCount === null || !semester
      ? null
      : targetCount * activeIndicatorIds.length;
    ...
    console.error('GET /api/obe/achievement/graduation-requirement error:', error);
