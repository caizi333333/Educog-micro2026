...
export function HyperAnalyticsPage() {
  ...
  return (
    <div className="-m-6 min-h-[calc(100vh-3.5rem)] overflow-auto bg-[#070a0d] text-slate-100 animate-fade-in">
      ...
      <div className="mx-auto max-w-7xl px-4 py-5 md:px-6">
        <section className="mb-6 grid gap-3 md:grid-cols-4 stagger-children">
          {[
            ...
            <div key={label as string} className="glass-hover transition-all rounded-md border border-white/[0.08] bg-white/[0.035] p-4">
              <div className="chip-mark flex h-8 w-8 items-center justify-center rounded-md"><Icon className="h-4 w-4 text-cyan-100" /></div>
              <div className="mt-3 font-mono text-2xl font-semibold text-slate-50 stat-glow">{value as string}</div>
        ...
        <main className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          <section className="glass-hover rounded-md border border-white/[0.08] bg-white/[0.035]">
            <div className="border-b border-white/[0.08] p-5">
              ...
              <p className="mt-1 text-xs text-slate-500">基于测评与学习记录实时计算的知识点掌握情况。</p>
            </div>
            <div className="p-5">
              {knowledgeMastery.some((item) => Math.round(item.mastery) > 0) ? (
                ...
                <EmptyState centered icon={BrainCircuit} title="暂无知识点掌握度数据" description="完成一次在线测评后，这里会生成各知识点的掌握度热力图" className="min-h-64" />
              )}
            </div>
          </section>
          ...
          <section className="glass-hover rounded-md border border-white/[0.08] bg-white/[0.035]">
            <div className="border-b border-white/[0.08] p-5">
              ...
              <div className="relative mt-4">
                ...
                <Input
                  ...
                  placeholder="搜索学生..."
                  className="h-10 border-white/[0.09] bg-black/25 pl-10 text-slate-100 placeholder:text-slate-500 hover:border-cyan-300/30 transition-all focus-visible:ring-cyan-300/70"
                />
              </div>
            </div>
            ...
            <div className="p-5">
              {rankedStudents.length ? (
                ...
                <EmptyState centered icon={Award} title="暂无可展示的班级排行数据" className="min-h-64" />
              )}
            </div>
          </section>
        </main>
